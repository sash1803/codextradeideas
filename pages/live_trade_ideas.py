...
def refresh_pending_order_pre_activation(
    ...
    """
    Refresh pending SL/TPs using ONLY info available up to ts_prev.
    This prevents lookahead on the activation candle.
    """

    # Need ATR at ts_prev
    atr_prev = float(df.at[ts_prev, "atr"]) if "atr" in df.columns else np.nan
    if not np.isfinite(atr_prev):
        return pending_order

    # Composite levels at ts_prev (history-only)
    try:
        VAL_p = float(val_series.loc[ts_prev]) if np.isfinite(val_series.loc[ts_prev]) else np.nan
    except Exception:
        VAL_p = np.nan
    try:
        POC_p = float(poc_series.loc[ts_prev]) if np.isfinite(poc_series.loc[ts_prev]) else np.nan
    except Exception:
        POC_p = np.nan
    try:
        VAH_p = float(vah_series.loc[ts_prev]) if np.isfinite(vah_series.loc[ts_prev]) else np.nan
    except Exception:
        VAH_p = np.nan
    ...
    # Targets: long -> [POC, VAH], short -> [POC, VAL]
    targets = [POC_p, VAH_p] if side == "long" else [POC_p, VAL_p]

    plan = level_to_level_plan_safe(
        entry_px=entry,
        side=side,
        targets=targets,
        atrv=atr_prev,
    ...
    # Optional: clamp SL to the prior bar extreme (still no lookahead)
    ...
    if side == "long" and np.isfinite(sig_low):
        plan["sl"] = min(float(plan["sl"]), sig_low)
    elif side == "short" and np.isfinite(sig_high):
        plan["sl"] = max(float(plan["sl"]), sig_high)

    pending_order["sl"] = float(plan["sl"])
    pending_order["tp1"] = float(plan["tp1"])
    pending_order["tp2"] = float(plan["tp2"])
    pending_order["last_refresh_ts"] = ts_prev
...
def get_monday_range_completed(df: pd.DataFrame, current_time: pd.Timestamp) -> tuple[float, float]:
    ...
    # This week's Monday start (00:00)
    this_mon_start = (current_time - pd.Timedelta(days=current_time.dayofweek)).normalize()
    this_mon_end   = this_mon_start + pd.Timedelta(days=1)  # include Tue 00:00 (right-labeled bars)

    # If we're still in Monday, use PREVIOUS week's Monday (completed)
    if current_time < this_mon_end:
        mon_start = this_mon_start - pd.Timedelta(days=7)
    else:
        mon_start = this_mon_start
...
def backtest_xo_logic(df: pd.DataFrame, comp: Dict[str, Any], rng: Dict[str, Any], params: Dict[str, Any], force_eod_close: bool = False):
    ...
    def _sweep_cluster_low(df: pd.DataFrame, level: float, end_idx: int, max_lookback: int):
    ...
    def _sweep_cluster_high(df: pd.DataFrame, level: float, end_idx: int, max_lookback: int):
        start = max(0, end_idx - max_lookback + 1)
        j = end_idx
        max_hi = -float("inf")
        seen = False
    ...
    def _pick_targets_long(entry_px: float, candidates: list[float]):
        ups = sorted([x for x in candidates if pd.notna(x) and float(x) > entry_px])
        tp1 = ups[0] if len(ups) >= 1 else np.nan
        tp2 = ups[1] if len(ups) >= 2 else np.nan
        return tp1, tp2

    def _pick_targets_short(entry_px: float, candidates: list[float]):
        dns = sorted([x for x in candidates if pd.notna(x) and float(x) < entry_px], reverse=True)
    ...
    for i in range(len(df)):
        ...
        if open_pos is None:
            ...
            for level in support_levels:
                ...
                structure_sl = _sweep_cluster_low(df, level, end_idx=i-1, max_lookback=lookback_deviation)
                ...
                tp1, tp2 = _pick_targets_long(entry_px, [POC_p, VAH_p, mon_h])
                if not np.isfinite(tp1): continue

                min_tp_dist = entry_px * (min_tp1_bps / 10000.0)
                if abs(tp1 - entry_px) < min_tp_dist: continue

                reward = tp1 - entry_px
                if (reward / risk) < min_rr: continue
                if not np.isfinite(tp2): tp2 = entry_px + 2.0 * risk
                ...
                if filters_func is not None:
                    ...
                    if not filters_func(ts, "long", entry_px, f"XO Reclaim {level:.2f}", df, comp, rng, row):
                        continue

                open_pos = {
                    "side": "long", "entry_time": ts, "entry": entry_px, "sl": float(structure_sl),
            ...
            if open_pos is None:
                for level in resistance_levels:
                    ...
                    structure_sl = _sweep_cluster_high(df, level, end_idx=i-1, max_lookback=lookback_deviation)
                    if not np.isfinite(structure_sl): continue
                    ...
                    tp1, tp2 = _pick_targets_short(entry_px, [POC_p, VAL_p, mon_l])
                    if not np.isfinite(tp1): continue
                    ...
                    # --- CHECK FILTERS ---
                    if filters_func is not None:
                         if not filters_func(ts, "short", entry_px, f"XO Lost {level:.2f}", df, comp, rng, row):
                            continue

                    open_pos = {
                        "side": "short", "entry_time": ts, "entry": entry_px, "sl": float(structure_sl),
    ...
    if open_pos is not None and force_eod_close:
        ...
        trades.append({
            "entry_time": open_pos["entry_time"], "entry_kind": "market", "exit_time": ts_end,
    ...
    return trades_df, summary, open_pos, []
...
def build_filters_func(symbol: str, df: pd.DataFrame, n_bins: int, cfg: Dict[str, Any]):
    ...
    def _accepted(side: str, i_ts: pd.Timestamp, comp_in: dict) -> bool:
    ...
    def _filters_func(i_ts, side, entry_px, reason, df_in, comp_in, rng_in, row_now) -> bool:
        ...
        # 2) acceptance
        if not _accepted(side, i_ts, comp_in):
            return False
